...
describe('CASH IN API Tests', () => {
    ...
    const randomId = 'CSINID-' + Date.now();

    const requestBody = {
        "request-id": randomId,
        "command-id": "CSIN",
        "sender": "2250142373758",
        "destination": "2250102919300",
        "auth": "2222",
        "amount": "3000",
        "remarks": "",
        "extended-data": {
    ...
    context('Successful cashin transaction', () => {
        it('should process cashin transaction and return success response', () => {
            cy.request({
                method: 'POST',
                url: validEndpoint,
                body: requestBody,
                ...
                expect(response.body['command-id']).to.equal('CSIN');
                expect(response.body['request-id']).to.equal(randomId);
                expect(response.body.status).to.equal(0);
                expect(response.body['trans-id']).to.match(/^CSIN\d{6}\.\d{4}\.R\d{5}$/);
    ...
    context('Failed response', () => {
        ...
        it('should return not authorized sender number message', () => {
            const notAuthorizedsender = {
                "request-id": randomId,
                "command-id": "CSIN",
                "sender": "2250142373758", //not authorized sender number
                "destination": "2250102919300",
                "auth": "2222",
                "amount": "1233",
                "remarks": "",
            ...
            cy.request({
                ...
                expect(response.body['command-id']).to.equal('CSIN');
                expect(response.body['request-id']).to.equal(randomId);
                expect(response.body.status).to.equal(5);
                expect(response.body['trans-id']).to.match(/^CSIN\d{6}\.\d{4}\.R\d{5}$/);
